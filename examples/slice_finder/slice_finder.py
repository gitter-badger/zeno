...
class SliceFinder:
    ...
    def find_slice(self, k=50, epsilon=0.2, degree=3, risk_control=True, max_workers=1):
        ...
        reference = self.evaluate_model(self.data)
    ...
    def evaluate_model(self, data):
        ...
        y_tmp = y.to_numpy()
        y_tmp = list(map(functools.partial(np.expand_dims, axis=0), y_tmp))

        X_id = X[self.id_colname].tolist()
        df_tmp = data_full.loc[data_full[self.id_colname].isin(X_id)]
        y_p = df_tmp[self.output_colname].to_numpy()
        y_p = list(map(functools.partial(np.expand_dims, axis=0), y_p))

        return self.metric_fn(y_tmp, y_p)
        
        # if metric == log_loss:
        #     return list(map(functools.partial(metric, labels=self.model.classes_), y_tmp, y_p))
        # elif metric == accuracy_score:
        #     m_tmp = list(map(metric, y_tmp, y_p))

        # m_tmp = [1-i for i in m_tmp]
